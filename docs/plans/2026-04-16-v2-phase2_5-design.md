...
1. **goose crate 发布状态** — 需要先 `cargo search goose` 确认，决定 W1 走 thin wrap 还是 subprocess-via-MCP。
...
4. **goose 能否接受 managed hooks 注入** — Phase 2.5 W1 核心功能；若 goose 无此 API，thin wrap 方案退化。
