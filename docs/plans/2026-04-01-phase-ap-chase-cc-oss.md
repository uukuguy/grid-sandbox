...
| AP-D5 | Session Memory 持续提取（每轮而非 session-end） | T6 CompactionPipeline 完成 | ✅ 已补 |
...
| AP-D11 | Bash AST 命令分析增强（CC bashClassifier 风格） | T8 PermissionEngine 完成 | ✅ 已补 |
| AP-D12 | plan_mode 工具（enter_plan_mode/exit_plan_mode） | T8 PermissionEngine 完成 | ✅ 已补 |
| AP-D13 | tool_search 工具（大量 MCP 工具时的搜索） | 按需 | ⏳ |
| AP-D14 | ask_user 工具（结构化提问） | ApprovalGate 扩展 | ⏳ |
| AP-D15 | toolUseSummary 工具执行摘要（上下文降级注入） | T6 CompactionPipeline 完成 | ✅ 已补 |
