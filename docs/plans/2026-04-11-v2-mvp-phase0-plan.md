...
| ID | 内容 | 前置条件 | 状态 |
|----|------|---------|------|
| D1 | `grid-runtime/src/harness.rs` 接入 `payload.policy_context` (P1) → L3 managed hooks attach | S3.T3 `eaasp-l3-governance` 实现 Contract 5 `POST /v1/sessions/{id}/validate` ✅ (done @ 0907d13) | ✅ 已补（2026-04-12，S4.T2 4b-lite）`crates/grid-runtime/src/harness.rs::initialize` 读 `payload.policy_context.org_unit` / `.policy_version` / `.hooks.len()` 并通过 `tracing::info!` 结构化字段输出。**注意**：这是 read-only 观测性，hook 执行仍在 managed_hooks_json 路径，scoped-hook executor 见 D50/D53。Python 侧 `claude-code-runtime-python/src/claude_code_runtime/service.py::Initialize` 镜像同样 log line。集成测试 `initialize_preserves_policy_context_metadata` + Python `test_initialize_injects_memory_refs_preamble` 验证字段可达。 |
| D2 | `grid-runtime/src/harness.rs` 接入 `payload.memory_refs` (P3) → L2 memory projection 注入 engine context | S3.T2 `eaasp-l2-memory-engine` 提供 `memory_read` MCP tool + `POST /api/v1/memory/search` ✅ | ✅ 已补（2026-04-12，S4.T2 4b-lite）`crates/grid-runtime/src/harness.rs::initialize` 构建 `build_memory_preamble` helper，生成 `"## Prior memories from previous sessions\n\n- [{memory_type}] {content}\n..."` 前缀，作为 `MessageRole::System` ChatMessage 注入 `runtime.start_session()` 的 `initial_history`。Python 侧 `claude_code_runtime/service.py::Send` 使用 `session.preamble_injected` 一次性 guard 将同样前缀 prepend 到 `system_prompt`（Option 1 —— SkillLoader 一致的注入通道）。Rust 3 个集成测试 + 2 个 build_memory_preamble 单测 + Python 2 个测试覆盖。**注意**：runtime 侧只证明字段可达 + 前缀构建正确；LLM 是否真消费 memory_refs 影响输出需 D54 L4→L1 真 gRPC 通路后的行为测试（见 ADR-V2-004）。 |
...
### 来源：2026-04-12 S4.T2 4b-lite implementation + reviewer findings

S4.T2 `verify-v2-mvp` 实现采用 4b-lite 模式（见 `docs/design/EAASP/adrs/ADR-V2-004-l4-to-l1-real-grpc-binding.md`）。D1 + D2 真接入两个 L1 runtime；L4 保持 stub；verify script 在 L4 空洞处直接 POST L2/L3 REST 以通过 15 条断言。Reviewer APPROVE-WITH-COMMENTS：M1 (HasField proto3 presence) + M2 (DB 清理) + N1 (trap signals) 已 inline 修复，Minors 作为下列新 Deferred 记录。

| ID | 内容 | 前置条件 | 状态 |
|----|------|---------|------|
| D54 | **L4→L1 real gRPC binding**（原 D27 精化，supersedes D27） —— `tools/eaasp-l4-orchestration/session_orchestrator.create_session` 把 `RUNTIME_INITIALIZE_STUBBED` 事件占位替换为真调用 L1 runtime 的 `Initialize(SessionPayload) → SessionHandle`，`api.send_message` 替换为流式反流 L1 `Send(UserMessage) → ResponseChunk`。拆成 5 个子项 D54.a-e：(a) proto→Python 生成 pb2 + pb2_grpc 到 `tools/eaasp-l4-orchestration/src/eaasp_l4_orchestration/_proto/`；(b) 新建 `l1_client.py` 的 `L1RuntimeClient` 抽象；(c) `create_session` 接入 real initialize + 写 `RUNTIME_INITIALIZED` / `RUNTIME_INITIALIZE_FAILED` 事件 + session state 机状态转换（配合 D40）；(d) `send_message` 接入流式响应；(e) 新 integration test `tests/integration/test_l4_l1_grpc.py` 用真 runtime 跑完整回路 | Phase 1：proto Python 生成管线 + `grpcio` 依赖加入 L4 pyproject + 两个 runtime 容器化 fixture + D40 sessions state machine 实施。D54 完成后 `verify-v2-mvp.py` 可去掉所有 `# L4-STUBBED` 注释。 | ⏳ |
| D55 | `claude-code-runtime-python/service.py` 其他 proto3 submessage presence 检查路径（`event_context` / `EventContext` 相关分支）同样应使用 `HasField` 而非 truthy fallback。reviewer M1 fix 已覆盖 `policy_context` + `event_context`，但如果后续有新 submessage 字段加入需统一规则 | 加入 `has_field` 辅助或 lint rule 强制，防止回归 | ⏳ |
| D56 | `scripts/verify-v2-mvp.sh` 只清 `data/verify-v2-l*.db`；如果 L2/L3/L4 未来支持其他 persistent state (e.g. Redis cache, file-based WAL outside data/) 会有遗漏。目前只在 SQLite 层清理 | 持久化后端发生变化时同步更新 wipe 逻辑 | ⏳ |
| D57 | `crates/grid-runtime/tests/harness_payload_integration.rs` 定义了 `expected_preamble()` 辅助复制了 `GridHarness::build_memory_preamble` 的格式逻辑（blueprint N2）。`build_memory_preamble` 目前是 `pub(crate)`，跨 `tests/` 目录不可见 | 升级为 `pub fn` 后测试可直接调用，消除重复 | ⏳ |
| D58 | `lang/claude-code-runtime-python/tests/test_service.py::test_initialize_injects_memory_refs_preamble` 仅验证 `memory_refs` 被正确 project 到 Session dataclass，没有真正调用 `service.Send(...)` 走一遍 system_prompt prepend 路径（blueprint N4）。一个真正的 Send-path 测试需要用 `SdkWrapper` 的构造注入替身来捕获 `sdk.send_message` 的 `system_prompt` 参数 | SdkWrapper 已有构造注入 seam，低成本可补；放在 D58 避免 S4.T2 scope 膨胀 | ⏳ |
| D59 | `Makefile::mcp-orch-start` 仍硬编码 `--port 8082`（blueprint N5）。这是 S3.T4.5 端口规则 `默认 ≥10000` 的最后一个 straggler；应改成 `--port 18082` 且增加 `EAASP_MCP_ORCHESTRATOR_PORT` env 覆盖 | 与 skill-registry 同等处理（S3.T4.5 @ 85c5c6e 已做过一次，本次漏掉） | ⏳ |
| D60 | `scripts/verify-v2-mvp.py` assertion 11 只检查 `len(memory_refs) > 0` 就 PASS；`memory_id_1 in matched_ids` 的严格比对被降级为 non-fatal note（blueprint N7）。原因是 L2 FTS hybrid search 对短中文 intent 文本的排名非确定性 | L2 hybrid search 对 `"Transformer-001"` 设备 ID 做确定性排名后，把 assertion 11 升级为硬 failure | ⏳ |
| D61 | `scripts/assets/threshold-calibration-skill.md` fixture 硬编码 `version: 0.1.0`。如果 skill registry 后续改版本管理语义（e.g. 自动 bump），verify script 的 `skill promote TEST_SKILL_ID 0.1.0` 会 drift（blueprint N6） | assertion 2 改为先从 `skill submit` 响应里解析版本号，再传给 `skill promote`，消除对 fixture 常量的依赖 | ⏳ |

**检查纪律：** S3 每个 Task 开始前先 `grep "⏳" docs/plans/2026-04-11-v2-mvp-phase0-plan.md`，确认是否有条件已达成的项可以顺手补齐。S3.T2 完成后检查 D2，S3.T3 完成后检查 D1，**S3.T4 完成后**：D1/D2/D6 前置条件均已满足但仍留至专门的 harness 接入 task（不在 S3.T5 CLI 范围内）；S3.T5 之后检查 D8/D10/D20 —— skill-registry 和 memory-engine 的 MCP facade 是否能统一迁移；S4 开始前检查 D6/D24（Pyright workspace config）；**S4.T2 完成后**：D1 + D2 ✅，D27 由 D54 精化接替（D27 保留 ⏳ 作为历史记号，正式工作在 D54）。
