...
| **D140** | Phase 2.5 S0.T3 — grid-runtime envelope-mode dispatch sites 未调 `HookContext::with_event` | 🟡 P1-defer | **归属 Phase**: Phase 2.5 W1 前置（小型热修）。**归属源码**: `crates/grid-engine/src/agent/harness.rs` — `fire_post_task_hooks` + `dispatch_stop_hooks` 两处调用点。**归属 Contract Test**: `tests/contract/contract_v1/test_hook_envelope.py` （grid-runtime 0/5 xfail）。**现象**: S0.T3 已为 `HookContext` 添加 `with_event/with_skill_id/with_memory_ids` builder path（ADR-V2-006 §2/§3 字段 + `GRID_EVENT`/`GRID_SKILL_ID` env），但 engine 两处 dispatch 调用点仍走 legacy 全量 struct 投影 — stdin envelope JSON 缺 `event` / `skill_id` 字段，env 缺 `GRID_EVENT` / `GRID_SKILL_ID`。**对比**: Python 侧自 Phase 2 S3.T5（@ `7cb48eb`）起已合规；仅 grid 落后。**修复路径**: 3-5 LOC — 在两处 dispatch 前 `ctx = ctx.with_event(<event_name>).with_skill_id(<sid>)`。**验证**: D140 fix 后 `pytest tests/contract/contract_v1/test_hook_envelope.py --runtime=grid` 从 0/5 → 5/5 PASS。衍生自 Phase 2.5 S0.T4 xfail root cause |
...
| ID | 标题 | 引入 | 状态 | 去向 |
|----|------|------|------|------|
| **D145** | session_orchestrator.py `delta_buf` + `ctype == "text_delta"` 在 `send_message` / `stream_message` 重复 | Phase 3.5 S2.T1 review | 🧹 tech-debt | 抽 shared helper；S2.T2 已关闭 CLI 侧 |
...
| **D147** | Python proto3 enum `.pyi` stub 声明 `ChunkType \| str \| None` 拒绝 int，但 runtime 接受 — Pyright strict mode 噪音 | Phase 3.5 S0 → S1 diagnostics | 🧹 tech-debt | 上游 grpcio-tools stub generator 改进 / 或重新生成带 IntFlag 扩展的 stub |
...
**合计新增：6 项 Deferred（2 🟡 P1-active + 4 🧹 tech-debt）**
