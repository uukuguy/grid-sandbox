...
| OpenCode | TS | ✅ **源码验证完毕 (R1, 2026-04-12)**：确认 **T1**。per-tool `tool.execute.before/after` + 独立 Permission Allow/Deny/Ask + MCP 3 transport + SKILL.md frontmatter。Adapter 3-4 天。详见 `L1_RUNTIME_R1_OPENCODE_EVAL.md` |
...
| **Agno 2.0 / AgentOS** | Python | ✅ **源码验证完毕 (R2, 2026-04-12)**：确认维持 **T2**。MCP+Skills 完全达标（甚至超标），但 Hooks 是 agent-run 级别非 per-tool — `pre_hooks/post_hooks` 在 `agent.run()` 入口/出口各一次，签名无 `tool_name` 参数。Adapter 5-7 天。详见 `L1_RUNTIME_R2_AGNO_EVAL.md` |
...
| **Python + 传统 AI/ML** | Pydantic AI (T3) / **Agno 2.0 (T2 ✅ 源码验证)** / LangGraph (T3) | ⚠️ Agno tier 已确认，adapter 未交付 |
| **Rust 系统团队** | Goose (T2) / Grid 自研 | ⚠️ Goose 未交付；Grid 是 EAASP 内核不是独立 L1 候选 |
| **TypeScript 团队** | CCB (T1?) / **OpenCode (T1 ✅ 源码验证)** | ⚠️ OpenCode tier 已确认，adapter 未交付 |
