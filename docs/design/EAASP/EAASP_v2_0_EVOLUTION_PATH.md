...
| **D87** | **grid-engine agent loop 过早终止（多步工作流 bug）** | E2E 2026-04-14 发现：grid-runtime 在 threshold-calibration skill 下只执行 step 1 (`scada_read_snapshot`) 后停止，跳过 step 2-6（memory_search / memory_write_anchor / memory_write_file），而 claude-code-runtime 自主完成全部 6 步。**根因**: `crates/grid-engine/src/agent/harness.rs:1169` 判断条件 `stop_reason != ToolUse \|\| tool_uses.is_empty()` 过于激进——LLM 返回任意非 tool_use 的 text 就退出 loop。**对比**: Anthropic SDK 把 tool_result 塞进 UserMessage 再重新调 LLM，让 LLM 自行决定是否继续。**修复方向** (Option 3): tool_result 作为 UserMessage 追加 → re-invoke LLM → 只在 `stop_reason=EndTurn` 或显式完成标记时退出。**影响**: Phase 0.5 MVP 时被简单 skill 掩盖，Phase 1 Event Engine 事件流暴露。不阻塞 Phase 1 观察力验收。回归测试已锁定 @ `crates/grid-engine/tests/d87_multi_step_workflow_regression.rs`。→ **Phase 2 首个任务** |
| **D88** | **hermes-runtime stdio MCP 支持缺失 — LLM 陷入幻觉式工作流** | E2E 2026-04-14 发现：hermes-runtime 下 0 个 PRE_TOOL_USE，但 LLM 流式输出"让我先检查 mock-scada 的实现方式"→"让我尝试模拟完整的阈值校准工作流程"→输出**虚构的 SCADA 数据和分析**。**根因**: `lang/hermes-runtime-python/src/hermes_runtime/service.py:374-382` ConnectMcp 对 stdio transport 的 server 只 `logger.warning` 然后**仍上报 connected=True**，但 `inject_mcp_tools` 只在 SSE/HTTP bridges 存在时调用（行 384 `if bridges`）。结果：hermes 向 L4 谎报 MCP 连接成功（SESSION_MCP_CONNECTED 事件出现），但 agent 实际没有 tool 可用，LLM 只能凭空编数据。**对比**: grid-runtime 和 claude-code-runtime 都通过 L4 ConnectMcp 获得 stdio MCP 配置并能真实调用。**修复方向**: (a) hermes 支持 stdio MCP transport（容器化方案需要 subprocess），或 (b) hermes 诚实上报 failed=True，L4 应将 transport 不兼容作为硬错误。**严重度**: 比 D87 更危险——不是跳步骤，是**产出虚假数据冒充真实证据**，违反 v2.0 §20 Anti-Pattern 14 (Agent overreach)。→ **Phase 2 高优先级任务** |
