...
| **staging** | 强制容器隔离 | 受限（不透传 API key） | 破坏性操作需审批 | 警告级 | 60s | 集成测试/预发布验证 |
...
**关键规则**：
- RunMode=Sandboxed 时，**无论** Profile 是什么，都直接本地执行（容器已提供隔离）
- RunMode=Host + Development 时，零摩擦本地执行
- RunMode=Host + Staging 时，强制容器执行，后端不可用则报错
- RunMode=Host + Production 时，强制容器执行，后端不可用则报错
...
后端优先级：Docker → WASM → External。Staging/Production 均不允许回退到 Subprocess 本地执行。
...
- **Staging**：强制容器隔离，审计级别为警告级，超时 60 秒
- **Production**：强制容器隔离，审计级别为完整，超时 30 秒，所有操作需审批
- 两者都不允许降级到本地执行——后端不可用时直接报错
