...
// ── ADR/DDD Architecture Change Detection ──────────────────────────────────
...
function detectArchChange(file) {
  ...
  return { isArch: false, category: null };
}

/**
 * recordArchChange(file, category) — Accumulate architecture changes for batch doc generation.
 */
function recordArchChange(file, category) {
  ensureDataDir();
  let pending = readJSON(ADR_SIGNAL_PATH) || { changes: [], createdAt: Date.now() };
  // Dedup by file
  if (!pending.changes.some(c => c.file === file)) {
    pending.changes.push({ file, category, timestamp: Date.now() });
    fs.writeFileSync(ADR_SIGNAL_PATH, JSON.stringify(pending, null, 2), 'utf-8');
  }
  return pending.changes.length;
}

/**
 * consumeArchChanges() — Read and clear pending architecture changes.
 * Returns array of changes, or null if none.
 */
function consumeArchChanges() {
  if (!fs.existsSync(ADR_SIGNAL_PATH)) return null;
  const pending = readJSON(ADR_SIGNAL_PATH);
  if (!pending || !pending.changes || pending.changes.length === 0) return null;
  // Clear the signal file
  fs.unlinkSync(ADR_SIGNAL_PATH);
  return pending.changes;
}

module.exports = { init, getContext, recordEdit, feedback, consolidate, stats, detectArchChange, recordArchChange, consumeArchChanges };
