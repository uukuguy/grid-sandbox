...
use grid_engine::agent::{run_agent_loop, AgentEvent, AgentLoopConfig};
use grid_engine::providers::{create_provider, Provider};
...
impl EvalRunner {
    ...
    pub async fn run_task(&self, task: &dyn EvalTask) -> Result<TaskResult> {
        ...
        let mut base_registry = grid_engine::tools::default_tools();
        ...
        let effective_provider: Arc<dyn grid_engine::providers::Provider> =
        ...
        let tool_ctx = grid_types::ToolContext {
    ...
    async fn run_task_cli(
        ...
        let output = AgentOutput {
            ...
            tool_calls: cli_output
                ...
                .collect(),
            messages: vec![grid_types::ChatMessage::assistant(&cli_output.text)],
    ...
    async fn run_task_server(
        ...
        let output = match msg_resp {
            ...
            Ok(resp) => {
                ...
                let body = resp.text().await.unwrap_or_default();
                AgentOutput {
                    messages: vec![grid_types::ChatMessage::assistant(&format!(
                        "Server error ({}): {}",
                        status, body
                    ))],
                    ..AgentOutput::default()
    ...
    fn load_agent_manifest(path: &str) -> Result<grid_engine::agent::entry::AgentManifest> {
        ...
        let manifest: grid_engine::agent::entry::AgentManifest = serde_yaml::from_str(&content)?;
        Ok(manifest)
    }
    ...
    async fn run_task_docker(
        ...
        use grid_engine::sandbox::{DockerAdapter, RuntimeAdapter};
        ...
        let mut sandbox_config = grid_engine::sandbox::SandboxConfig::new(
            grid_engine::sandbox::SandboxType::Docker,
        );
        ...
        let output = match result {
            Ok(exec_result) => {
                ...
                AgentOutput {
                    ...
                    tool_calls: vec![],
                    messages: vec![grid_types::ChatMessage::assistant(&messages_text)],
                    duration_ms: start.elapsed().as_millis() as u64,
                }
            }
            Err(e) => {
