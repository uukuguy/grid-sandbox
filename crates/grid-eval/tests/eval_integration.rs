...
use grid_engine::providers::Provider;

use grid_eval::config::EvalConfig;
use grid_eval::mock_provider::MockProvider;
use grid_eval::recorder::{EvalRecorder, EvalTrace};
use grid_eval::reporter::Reporter;
use grid_eval::runner::{EvalReport, EvalRunner, TaskResult};
use grid_eval::score::{EvalScore, ScoreDetails};
use grid_eval::scorer::{BehaviorScorer, ExactMatchScorer, Scorer, ToolCallScorer};
use grid_eval::task::{AgentOutput, Difficulty, EvalTask, TaskMetadata, ToolCallRecord};
...
impl EvalTask for SimpleTextTask {
    ...
    fn available_tools(&self) -> Option<Vec<grid_types::tool::ToolSpec>> {
        None
    }
...
fn test_scoring_pipeline_text_match() {
    ...
    let output = AgentOutput {
        messages: vec![grid_types::ChatMessage::assistant("The answer is 42")],
        ..AgentOutput::default()
    };
...
fn test_recorder_extract_interactions() {
    let interaction = grid_eval::recorder::record_interaction(
        "user: hello",
        "resp-1",
        "Hello!",
        "end_turn",
...
async fn test_mock_provider_call_tracking() {
    ...
    let req = || grid_types::CompletionRequest {
        model: "test".into(),
        system: None,
        messages: vec![],
        max_tokens: 100,
...
async fn test_mock_provider_tool_then_text_sequence() {
    ...
    assert!(matches!(
        r1.stop_reason,
        Some(grid_types::StopReason::ToolUse)
    ));
    match &r1.content[0] {
        grid_types::ContentBlock::ToolUse { name, input, .. } => {
    ...
    assert!(matches!(
        r2.stop_reason,
        Some(grid_types::StopReason::EndTurn)
    ));
    match &r2.content[0] {
        grid_types::ContentBlock::Text { text } => {
...
fn test_full_pipeline_score_report_record() {
    ...
    let output_fail = AgentOutput {
        messages: vec![grid_types::ChatMessage::assistant("I don't know")],
        input_tokens: 60,
        output_tokens: 15,
        rounds: 1,
        stop_reason: "EndTurn".into(),
...
fn test_auto_scorer_integration() {
    use grid_eval::scorer::auto_scorer;
    ...
    let scorer = auto_scorer(&def);
    let output = AgentOutput {
        messages: vec![grid_types::ChatMessage::assistant(
...
fn test_eval_config_defaults() {
    ...
    match &config.target {
        grid_eval::config::EvalTarget::Engine(engine) => {
...
fn test_timeout_score_details() {
    use grid_eval::score::ScoreDetails;
...
fn test_tool_allowlist_in_jsonl_task() {
    use grid_eval::datasets::loader::JsonlTask;
...
fn test_regression_detection_integration() {
    use grid_eval::reporter::{Reporter, RegressionReport};
