...
async fn show_config(state: &AppState) -> Result<()> {
    ...
    let entries = vec![
        ConfigEntry {
            ...
            value: std::env::var("OCTO_DB_PATH").unwrap_or_else(|_| "not set".to_string()),
...
async fn init_config(_state: &AppState) -> Result<()> {
    ...
    env_content.push_str(&format!("OCTO_DB_PATH={}\n", db_path));
...
async fn get_config(key: String, state: &AppState) -> Result<()> {
    let value = match key.as_str() {
        "LLM_PROVIDER" | "ANTHROPIC_API_KEY" | "OPENAI_API_KEY" | "OCTO_DB_PATH" | "RUST_LOG"
        | "OCTO_HOST" | "OCTO_PORT" => std::env::var(&key).ok(),
