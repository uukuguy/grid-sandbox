...
pub async fn execute_init(state: &AppState) -> Result<()> {
    ...
    // 1. Ensure all OctoRoot directories exist
    root.ensure_dirs()?;
    ...
    // 4. Ensure .octo/.gitignore includes config.local.yaml
...
# Use ~/.octo/credentials.yaml for API keys.
# Use .octo/config.local.yaml for local overrides (git-ignored).
