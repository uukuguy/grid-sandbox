...
fn cmd_config(path: &str) -> Result<()> {
    use grid_eval::config::EvalTomlConfig;
...
fn cmd_history(limit: usize, suite: Option<&str>, since: Option<&str>) -> Result<()> {
    use grid_eval::run_store::{RunFilter, RunStore};
...
fn cmd_report(run_id: &str, format: &str) -> Result<()> {
    ...
    match format {
        ...
        "markdown" => {
            if let Some(ref report) = run.report {
                let md = grid_eval::reporter::Reporter::to_markdown(report);
                println!("\n{}", md);
            } else {
                println!("\nNo detailed report available for this run.");
            }
...
fn cmd_trace(run_id: &str, task_id: &str, full: bool) -> Result<()> {
    use grid_eval::run_store::RunStore;
    use grid_eval::trace::TraceEvent;
...
fn cmd_diff(run_a: &str, run_b: &str) -> Result<()> {
    ...
    if let (Some(ref report_a), Some(ref report_b)) = (&a.report, &b.report) {
        let regression = grid_eval::reporter::Reporter::diff_report(report_b, report_a);
        ...
        let changes: Vec<_> = regression
            .task_diffs
            .iter()
            .filter(|d| d.status != grid_eval::reporter::DiffStatus::Unchanged)
            .collect();
        ...
        if !changes.is_empty() {
            ...
            for diff in changes {
                let status = match diff.status {
                    grid_eval::reporter::DiffStatus::Improved => "IMPROVED ",
                    grid_eval::reporter::DiffStatus::Regressed => "REGRESSED",
                    grid_eval::reporter::DiffStatus::New => "NEW      ",
                    grid_eval::reporter::DiffStatus::Removed => "REMOVED  ",
                    grid_eval::reporter::DiffStatus::Unchanged => continue,
                };
                let baseline = diff
                    .baseline_score
                    .map(|s| format!("{:.1}", s))
