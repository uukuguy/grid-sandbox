...
use anyhow::Result;
use grid_engine::agent::{AgentEvent, AgentMessage};
use grid_engine::AgentId;
use grid_types::{SessionId, UserId};
...
pub async fn execute_ask(opts: AskOptions, state: &AppState) -> Result<()> {
    ...
    let sandbox_id = session_store
        ...
        .await
        .map(|s| s.sandbox_id)
        .unwrap_or_else(|| grid_types::SandboxId::from_string("default"));
