...
use anyhow::Result;
use grid_types::{MemoryCategory, MemoryEntry, MemoryFilter, SearchOptions};
use serde::Serialize;
...
async fn search_memory(query: String, limit: usize, state: &AppState) -> Result<()> {
    ...
    let opts = SearchOptions {
        user_id: grid_types::id::DEFAULT_USER_ID.to_string(),
        limit,
        ..Default::default()
    };
...
async fn add_memory(content: String, tags: Option<String>, state: &AppState) -> Result<()> {
    ...
    let entry = MemoryEntry::new(grid_types::id::DEFAULT_USER_ID, category, &content);
