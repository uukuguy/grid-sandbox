...
use grid_engine::sandbox::{
    ExecutionTargetResolver, GridRunMode, SandboxProfile, SandboxRouter, ToolCategory,
};
...
fn run_mode_display(mode: &GridRunMode) -> &'static str {
    match mode {
        GridRunMode::Host => "host",
        GridRunMode::Sandboxed => "sandboxed (container)",
    }
}
...
mod tests {
    ...
    fn test_run_mode_display() {
        assert_eq!(run_mode_display(&GridRunMode::Host), "host");
        assert_eq!(run_mode_display(&GridRunMode::Sandboxed), "sandboxed (container)");
    }
