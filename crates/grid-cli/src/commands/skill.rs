        ...
        skills: skills
            ...
            .map(|s| {
                ...
                let triggers: Vec<String> = s
                    ...
                    .iter()
                    .map(|t| match t {
                        grid_types::skill::SkillTrigger::FilePattern { pattern } => {
                            format!("file:{}", pattern)
                        }
                        grid_types::skill::SkillTrigger::Command { command } => {
                            format!("cmd:{}", command)
                        }
                        grid_types::skill::SkillTrigger::Keyword { keyword } => {
                            format!("kw:{}", keyword)
                        }
                    })
                    .collect();
...
async fn validate_skill(path: String) -> Result<()> {
    ...
    match grid_engine::skills::loader::SkillLoader::parse_skill(&skill_file) {
        Ok(skill) => {
