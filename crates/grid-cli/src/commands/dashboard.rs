...
mod tests {
    ...
    async fn test_auth_enabled_invalid_token() {
        ...
        let _storage = grid_engine::auth::ApiKeyStorage::new(db_path.as_path()).unwrap();
    ...
    async fn test_auth_enabled_valid_token() {
        ...
        let storage = grid_engine::auth::ApiKeyStorage::new(db_path.as_path()).unwrap();
        let (stored_key, raw_key) = grid_engine::auth::StoredApiKey::generate("test-user", grid_engine::auth::Role::Admin);
    ...
    async fn test_chat_endpoint_with_valid_auth() {
        ...
        let db_path = tmp.path().join("test_keys.db");
        let storage = grid_engine::auth::ApiKeyStorage::new(db_path.as_path()).unwrap();
        let (stored_key, raw_key) = grid_engine::auth::StoredApiKey::generate("chat-user", grid_engine::auth::Role::User);
        storage.create(&stored_key).unwrap();
