...
pub struct TuiState {
    ...
    pub plan_steps: Vec<grid_engine::agent::dual::PlanStep>,
    ...
    // ── Custom commands ──
    /// Loaded custom commands from ~/.grid/commands/ and .grid/commands/.
    pub custom_commands: Vec<grid_engine::commands::CustomCommand>,
...
impl TuiState {
    ...
    pub fn last_assistant_response_text(&self) -> Option<String> {
        for msg in self.messages.iter().rev() {
            if msg.role != grid_types::message::MessageRole::Assistant {
                continue;
            }
