...
pub async fn run_tui_conversation(state: &AppState) -> Result<()> {
    use grid_types::{SandboxId, UserId};
...
fn handle_agent_event(state: &mut app_state::TuiState, event: grid_engine::agent::AgentEvent) {
    use grid_engine::agent::AgentEvent;
    ...
    match event {
        ...
        AgentEvent::InteractionRequested { request_id, request } => {
            ...
            let prompt = match &request {
                grid_engine::tools::interaction::InteractionRequest::Question { prompt, .. } => prompt.clone(),
                grid_engine::tools::interaction::InteractionRequest::Select { prompt, .. } => prompt.clone(),
                grid_engine::tools::interaction::InteractionRequest::Confirm { prompt } => prompt.clone(),
                grid_engine::tools::interaction::InteractionRequest::Message { content, .. } => content.clone(),
            };
            let label = match &request {
                grid_engine::tools::interaction::InteractionRequest::Message { .. } => "[message]",
                _ => "[ask_user]",
            };
...
fn handle_subagent_event(
    state: &mut app_state::TuiState,
    source_id: String,
    event: grid_engine::agent::AgentEvent,
) {
    use grid_engine::agent::AgentEvent;
