...
impl LoopGuardVerdict {
    /// Returns true if the verdict allows execution.
    pub fn is_allowed(&self) -> bool {
        matches!(self, LoopGuardVerdict::Allow)
    }
    ...
    /// Get the block message if this is a Block or CircuitBreak variant.
    pub fn block_message(&self) -> Option<&str> {
        match self {
            LoopGuardVerdict::Block(msg) => Some(msg),
            LoopGuardVerdict::CircuitBreak(msg) => Some(msg),
            _ => None,
        }
    }
}
...
impl LoopGuard {
    ...
    /// Reset the loop guard for a new agent execution.
...
mod tests {
    ...
    fn global_circuit_breaker() {
        ...
        let mut guard = LoopGuard::with_config(config);
        for i in 0..5 {
    ...
    fn test_reset() {
        ...
        guard.check("tool", &params);
        guard.record_outcome("tool", &params, "result");

        let stats_before = guard.stats();
        assert_eq!(stats_before.total_calls, 2);
