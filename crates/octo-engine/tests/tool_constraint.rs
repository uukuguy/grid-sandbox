...
fn make_skill(name: &str, allowed: Option<Vec<&str>>, denied: Option<Vec<&str>>) -> SkillDefinition {
    SkillDefinition {
        name: name.to_string(),
...
fn test_allowed_tools_restrict() {
    ...
    assert!(matches!(enforcer.check("file_write"), ConstraintResult::Denied(_)));
}
...
fn test_glob_pattern_match() {
    ...
    let enforcer = ToolConstraintEnforcer::from_active_skills(&skills);
    assert_eq!(enforcer.check("mcp:server:tool1"), ConstraintResult::Allowed);
    assert_eq!(enforcer.check("mcp:server:tool2"), ConstraintResult::Allowed);
    assert!(matches!(
...
fn test_merge_multiple_skills() {
    ...
    assert!(matches!(enforcer.check("bash"), ConstraintResult::Denied(_)));
    // file_read is allowed
    assert_eq!(enforcer.check("file_read"), ConstraintResult::Allowed);
    // unknown tool not in allowed list
    assert!(matches!(enforcer.check("exec"), ConstraintResult::Denied(_)));
}
