...
async fn main() -> Result<()> {
    ...
    // Use GRID_LOG=grid_engine=debug for explicit server-level debug logging.
    ...
    if let Some(ref pc_config) = cfg.provider_chain {
        let chain = Arc::new(grid_engine::providers::ProviderChain::new(
        ...
        for instance_config in &pc_config.instances {
            ...
            let instance = grid_engine::providers::LlmInstance {
        ...
        chain
            .start_health_checker(grid_engine::providers::HealthCheckConfig {
    ...
    let scheduler = if cfg.scheduler.enabled {
        ...
        let s = Scheduler::new(
            ...
            Some(agent_runtime.security_policy().clone()
                as std::sync::Arc<dyn grid_types::PathValidator>),
        );
        Some(Arc::new(s))
    } else {
        None
