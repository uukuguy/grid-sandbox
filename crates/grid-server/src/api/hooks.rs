...
async fn list_points() -> Json<HookPointsResponse> {
    // All HookPoint variants — kept in sync with grid_engine::hooks::HookPoint
    let points = vec![
        "PreToolUse",
        "PostToolUse",
        "PreTask",
...
async fn reload_declarative() -> Json<ReloadResponse> {
    let config = grid_engine::hooks::declarative::loader::load_hooks_config_auto(None);
    match config {
...
async fn list_wasm_plugins() -> Json<Vec<WasmPluginInfo>> {
    #[cfg(feature = "sandbox-wasm")]
    {
        ...
        let plugins = grid_engine::hooks::wasm::loader::discover_plugins(&[]);
        let result = plugins
            .into_iter()
