...
async fn submit_task(
    ...
    let config = grid_engine::scheduler::AgentTaskConfig {
...
fn map_execution_status(status: &grid_engine::scheduler::ExecutionStatus) -> TaskStatus {
    match status {
        grid_engine::scheduler::ExecutionStatus::Running => TaskStatus::Running,
        grid_engine::scheduler::ExecutionStatus::Success => TaskStatus::Success,
        grid_engine::scheduler::ExecutionStatus::Failed
        | grid_engine::scheduler::ExecutionStatus::Timeout
        | grid_engine::scheduler::ExecutionStatus::Cancelled => TaskStatus::Failed,
    }
}

/// Convert ScheduledTask to TaskResponse
fn scheduled_task_to_response(
    task: &grid_engine::scheduler::ScheduledTask,
    execution: Option<&grid_engine::scheduler::TaskExecution>,
) -> TaskResponse {
