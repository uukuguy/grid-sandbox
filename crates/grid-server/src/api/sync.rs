...
pub async fn sync_status(
...
) -> Result<Json<grid_engine::sync::SyncStatus>, StatusCode> {
    ...
    grid_engine::sync::server::standalone::ensure_tables(&conn)
        .await
        .map_err(|e| {
    ...
    let status = grid_engine::sync::server::standalone::status(&conn, &query.device_id)
        .await
        .map_err(|e| {
...
pub async fn sync_pull(
    State(state): State<Arc<AppState>>,
    Json(req): Json<grid_engine::sync::SyncPullRequest>,
) -> Result<Json<grid_engine::sync::SyncPullResponse>, StatusCode> {
    let conn = tokio_rusqlite::Connection::open(&state.db_path)
        .await
        .map_err(|e| {
    ...
    let resp = grid_engine::sync::server::standalone::pull(&conn, &req)
        .await
        .map_err(|e| {
...
pub async fn sync_push(
    State(state): State<Arc<AppState>>,
    Json(req): Json<grid_engine::sync::SyncPushRequest>,
) -> Result<Json<grid_engine::sync::SyncPushResponse>, StatusCode> {
    let conn = tokio_rusqlite::Connection::open(&state.db_path)
        .await
        .map_err(|e| {
    ...
    let resp = grid_engine::sync::server::standalone::push(&conn, &req)
        .await
        .map_err(|e| {
