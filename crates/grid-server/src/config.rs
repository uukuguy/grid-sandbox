...
impl Default for LoggingConfig {
    fn default() -> Self {
        Self {
            level: "grid_server=info,grid_engine=info,tower_http=info".to_string(),
...
/// Credentials file structure (`~/.grid/credentials.yaml`)
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
...
impl Config {
    ...
    /// 2. Global config: `~/.grid/config.yaml`
    /// 3. Project config: `$PWD/.grid/config.yaml`
    /// 4. Project local config: `$PWD/.grid/config.local.yaml`
    /// 5. Legacy fallback: `$PWD/config.yaml` (if no .octo configs found)
    /// 6. CLI arguments: --port, --host
    /// 7. Environment variables: GRID_*, ANTHROPIC_*, OPENAI_*
    ///
    ...
    pub fn load(
            ...
            if !grid_root.project_config().exists() && legacy_path.exists() {
                tracing::warn!(
                    "Found config.yaml at project root (legacy location). \
                     ...
                     mv config.yaml .grid/config.yaml"
                );
        ...
        // Server
        if let Ok(host) = std::env::var("GRID_HOST") {
            config.server.host = host;
        }
        if let Ok(port) = std::env::var("GRID_PORT") {
        ...
        // CORS strict mode
        if let Ok(strict) = std::env::var("GRID_CORS_STRICT") {
            config.server.cors_strict = strict.parse().unwrap_or(false);
        }
        // Request body size limit
        if let Ok(size) = std::env::var("GRID_MAX_BODY_SIZE") {
            if let Ok(n) = size.parse() {
                config.server.max_body_size = Some(n);
            }
        }
        // Request timeout
        if let Ok(timeout) = std::env::var("GRID_REQUEST_TIMEOUT_SECS") {
        ...
        // CORS origins
        if let Ok(origins) = std::env::var("GRID_CORS_ORIGINS") {
        ...
        // Database
        if let Ok(path) = std::env::var("GRID_DB_PATH") {
        ...
        if let Ok(fmt) = std::env::var("GRID_LOG_FORMAT") {
            config.logging.format = fmt;
        }

        // Working directory
        if let Ok(dir) = std::env::var("GRID_WORKING_DIR") {
            config.working_dir = Some(dir);
        }

        // Event bus
        if let Ok(enabled) = std::env::var("GRID_ENABLE_EVENT_BUS") {
            config.enable_event_bus = enabled.parse().unwrap_or(false);
        }

        // TLS
        if let Ok(enabled) = std::env::var("GRID_TLS_ENABLED") {
            config.tls.enabled = enabled.parse().unwrap_or(false);
        }
        if let Ok(path) = std::env::var("GRID_TLS_CERT_PATH") {
            config.tls.cert_path = Some(PathBuf::from(path));
        }
        if let Ok(path) = std::env::var("GRID_TLS_KEY_PATH") {
            config.tls.key_path = Some(PathBuf::from(path));
        }
        if let Ok(v) = std::env::var("GRID_TLS_SELF_SIGNED") {
            config.tls.self_signed = v.parse().unwrap_or(false);
        }

        // Auth: GRID_AUTH_MODE and GRID_API_KEY override config.yaml
        if let Ok(mode) = std::env::var("GRID_AUTH_MODE") {
            let m = match mode.to_lowercase().as_str() {
                "none" => Some(grid_engine::auth::AuthMode::None),
                "api_key" | "apikey" => Some(grid_engine::auth::AuthMode::ApiKey),
                "full" => Some(grid_engine::auth::AuthMode::Full),
                _ => None,
            };
            if let Some(m) = m {
        ...
        if let Ok(key) = std::env::var("GRID_API_KEY") {
            if !key.is_empty() {
                use grid_engine::auth::ApiKeyConfig;
                let keys = config.auth.api_keys.get_or_insert_with(Vec::new);
                keys.push(ApiKeyConfig {
                    key,
                    user_id: Some(
                        std::env::var("GRID_API_KEY_USER").unwrap_or_else(|_| "default".into()),
                    ),
        ...
        // Sessions config
        if let Ok(max) = std::env::var("GRID_SESSIONS_MAX_CONCURRENT") {
            if let Ok(n) = max.parse() {
                config.sessions.max_concurrent = n;
            }
        }
        if let Ok(timeout) = std::env::var("GRID_SESSIONS_IDLE_TIMEOUT") {
            if let Ok(n) = timeout.parse() {
                config.sessions.idle_timeout_secs = n;
            }
        }
        if let Ok(mode) = std::env::var("GRID_SESSIONS_MEMORY_ISOLATION") {
    ...
    pub fn generate_default_yaml() -> String {
        ...
        output.push_str("#   GRID_AUTH_MODE=none\n");
        output.push_str("#\n");
        output.push_str("# Option 2: API key auth\n");
        output.push_str("#   GRID_AUTH_MODE=api_key\n");
        output.push_str("#   GRID_API_KEY=your-secret-key     # key clients use in Authorization: Bearer header\n");
        output.push_str(
            "#   GRID_API_KEY_USER=dev            # optional user id (default: \"default\")\n",
        );
        output.push_str("#\n");
