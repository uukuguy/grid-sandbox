...
use grid_engine::{AgentEvent, AgentMessage};
...
async fn handle_socket(
    ...
    let session_id_typed = grid_types::SessionId::from_string(&session_id);
    let user_id = instance
        .workspace
        .as_ref()
        .map(|w| grid_types::UserId::from_string(&w.user_id))
        .unwrap_or_default();
    let sandbox_id = grid_types::SandboxId::from_string(&format!("platform-{}", instance_id));
