...
impl JwtConfig {
    ...
    pub fn from_env() -> Result<Self> {
        let secret = std::env::var("GRID_JWT_SECRET")
            .context("GRID_JWT_SECRET environment variable is required")?;

        if secret.len() < 32 {
            anyhow::bail!("GRID_JWT_SECRET must be at least 32 characters");
        }
