...
mod wasm_hook_tests {
    use grid_engine::hooks::wasm::host_impl::HookHostState;
    use grid_engine::hooks::wasm::loader::discover_plugins;
    use grid_engine::hooks::wasm::manifest::PluginManifest;
...
"#;
        ...
        assert_eq!(
            ...
            grid_engine::hooks::HookFailureMode::FailOpen
        );
        assert_eq!(
            m_closed.hook_failure_mode(),
            grid_engine::hooks::HookFailureMode::FailClosed
        );
    }
...
"#;
    ...
    use grid_engine::hooks::wasm::bindings::octo::hook::host::Host;
    ...
    fn test_get_secret_with_capability() {
        ...
        // Set a test env var
        std::env::set_var("GRID_TEST_SECRET_KEY_8492", "test-value");
        let result = state.get_secret("GRID_TEST_SECRET_KEY_8492".to_string());
        assert_eq!(result.unwrap(), "test-value");
        std::env::remove_var("GRID_TEST_SECRET_KEY_8492");
    }
...
"#;
        let config: grid_engine::hooks::declarative::HooksConfig =
        ...
        assert!(matches!(
            pre[0].actions[0],
            grid_engine::hooks::declarative::HookActionConfig::Command { .. }
        ));
        assert!(matches!(
            pre[0].actions[1],
            grid_engine::hooks::declarative::HookActionConfig::Wasm { .. }
        ));
        assert!(matches!(
            pre[0].actions[2],
            grid_engine::hooks::declarative::HookActionConfig::Prompt { .. }
        ));
    }
}
