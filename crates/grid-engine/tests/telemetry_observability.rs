...
use grid_engine::event::TelemetryBus;
...
async fn telemetry_bus_publishes_context_degraded() {
    ...
    bus.publish(grid_engine::event::TelemetryEvent::ContextDegraded {
    ...
    match &events[0] {
        grid_engine::event::TelemetryEvent::ContextDegraded { session_id, level } => {
...
async fn telemetry_bus_publishes_token_budget_updated() {
    ...
    bus.publish(grid_engine::event::TelemetryEvent::TokenBudgetUpdated {
        session_id: "test-session".to_string(),
        used: 50_000,
        total: 200_000,
        ratio: 0.25,
...
async fn telemetry_bus_publishes_loop_guard_triggered() {
    ...
    bus.publish(grid_engine::event::TelemetryEvent::LoopGuardTriggered {
        session_id: "test-session".to_string(),
    ...
    assert_eq!(events.len(), 1);
    match &events[0] {
        grid_engine::event::TelemetryEvent::LoopGuardTriggered { session_id, reason } => {
            assert_eq!(session_id, "test-session");
...
async fn telemetry_bus_subscriber_receives_all_event_types() {
    ...
    assert!(matches!(
        e1,
        grid_engine::event::TelemetryEvent::ContextDegraded { .. }
    ));
    ...
    assert!(matches!(
        e3,
        grid_engine::event::TelemetryEvent::LoopGuardTriggered { .. }
    ));
}
...
async fn telemetry_metrics_recorded_for_new_events() {
    ...
    let degradation_count = metrics.counter("grid.context.degradations.total").get();
    ...
    let used_gauge = metrics.gauge("grid.context.tokens.used").get();
    assert_eq!(used_gauge, 1000);

    let total_gauge = metrics.gauge("grid.context.tokens.total").get();
