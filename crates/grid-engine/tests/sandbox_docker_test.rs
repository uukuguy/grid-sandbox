...
use grid_engine::sandbox::{RuntimeAdapter, SandboxConfig, SandboxType};

#[tokio::test]
#[cfg(feature = "sandbox-docker")]
async fn test_docker_adapter_create() {
    use grid_engine::sandbox::DockerAdapter;
...
async fn test_docker_adapter_not_found() {
    use grid_engine::sandbox::{DockerAdapter, SandboxId};
...
async fn test_docker_adapter_without_feature() {
    use grid_engine::sandbox::{DockerAdapter, SandboxId, SandboxType};
...
async fn test_docker_adapter_default_image() {
    ...
    assert_eq!(adapter.image(), "grid-sandbox:base");
}
...
struct ContainerGuard {
    sandbox_id: Option<grid_engine::sandbox::SandboxId>,
}

impl ContainerGuard {
    fn new(id: grid_engine::sandbox::SandboxId) -> Self {
        Self {
            sandbox_id: Some(id),
        }
    }

    fn id(&self) -> &grid_engine::sandbox::SandboxId {
        self.sandbox_id.as_ref().unwrap()
    }

    /// Release ownership without triggering cleanup
    #[allow(dead_code)]
    fn release(mut self) -> grid_engine::sandbox::SandboxId {
...
fn require_docker() -> bool {
    let adapter = grid_engine::sandbox::DockerAdapter::new("alpine:latest");
    if !adapter.is_available() {
