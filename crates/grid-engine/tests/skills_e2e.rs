...
use grid_engine::skills::{
    sync_builtin_skills, builtin_skill_names, ExecuteSkillTool, SkillSelector,
    SkillRegistry, TrustManager,
};
use grid_engine::skills::loader::SkillLoader;
use grid_engine::Tool;
use grid_types::skill::{ExecutionMode, SkillDefinition, SkillTrigger, TrustLevel};
...
fn make_test_skill(name: &str, mode: ExecutionMode, triggers: Vec<SkillTrigger>) -> SkillDefinition {
    SkillDefinition {
        ...
        denied_tools: None,
        execution_mode: mode,
        source_type: grid_types::skill::SkillSourceType::ProjectLocal,
        max_rounds: 0,
        background: false,
    }
}
...
async fn test_execute_skill_tool_knowledge() {
    ...
    let ctx = grid_types::ToolContext {
        sandbox_id: grid_types::SandboxId::from_string("test"),
...
fn test_system_prompt_skill_index() {
    use grid_engine::context::NewSystemPromptBuilder;
