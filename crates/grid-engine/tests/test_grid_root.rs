//! Integration tests for GridRoot unified directory management.

use std::path::Path;

use grid_engine::root::{encode_project_key, GridRoot};
...
fn test_full_lifecycle() {
    ...
    assert!(db.to_string_lossy().ends_with("grid.db"));
...
fn test_env_override_db_path() {
    ...
    std::env::set_var("GRID_DB_PATH", &custom_db);
    std::env::set_var("GRID_GLOBAL_ROOT", tmp.path().join("g"));
