...
use grid_types::{CompletionRequest, CompletionResponse, StopReason, StreamEvent, TokenUsage};

use grid_engine::providers::chain::{ChainProvider, FailoverPolicy, LlmInstance, ProviderChain};
...
async fn test_stream_failover_marks_unhealthy() {
    ...
    assert!(
        matches!(
            health,
            grid_engine::providers::chain::InstanceHealth::Unhealthy { .. }
        ),
