...
use grid_engine::skills::trust::TrustManager;
use grid_engine::skills::{SkillLoader, SkillManager, SkillRegistry};

/// Helper: create a SKILL.md inside `<temp>/.grid/skills/<name>/SKILL.md`.
fn create_skill(temp: &TempDir, name: &str, desc: &str, always: bool, tags: &[&str]) {
    let skill_dir = temp.path().join(".grid").join("skills").join(name);
...
fn test_build_index_empty_registry() {
    let temp = TempDir::new().unwrap();
    // Create the .grid/skills directory but no skills.
    let skills_dir = temp.path().join(".grid").join("skills");
