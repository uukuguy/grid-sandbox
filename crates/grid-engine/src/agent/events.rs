...
use grid_types::{ChatMessage, RiskLevel, StopReason};
...
pub enum AgentEvent {
    ...
    ToolProgress {
        tool_id: String,
        tool_name: String,
        progress: grid_types::ToolProgress,
    },
...
/// This is distinct from grid_types::StopReason which is provider-level.
...
mod tests {
    ...
    fn test_tool_progress_event_serialization() {
        let event = AgentEvent::ToolProgress {
            ...
            tool_name: "bash".into(),
            progress: grid_types::ToolProgress::percent(0.5, "Processing...")
                .with_elapsed(1500),
        };
    ...
    fn test_tool_progress_complete_flag() {
        let progress = grid_types::ToolProgress::percent(1.0, "Done");
        assert!(progress.is_complete());

        let progress_half = grid_types::ToolProgress::percent(0.5, "Half");
        assert!(!progress_half.is_complete());

        let indeterminate = grid_types::ToolProgress::indeterminate("Working...");
