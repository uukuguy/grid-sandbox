...
impl Default for AuthConfig {
    fn default() -> Self {
        let hmac_secret = std::env::var("GRID_HMAC_SECRET").unwrap_or_else(|_| {
            tracing::warn!(
                "GRID_HMAC_SECRET is not set. Using insecure default HMAC secret. \
...
impl AuthConfig {
    ...
    /// auth mode is configured but `GRID_HMAC_SECRET` was not set (falling
    ...
    pub fn warn_if_insecure(&self) {
        if self.mode == AuthMode::None {
            ...
            panic!(
                "GRID_HMAC_SECRET is not set but authentication is enabled (mode={:?}). \
                 The hardcoded default HMAC secret must NOT be used in production because \
                 it allows API key hash forgery. Set GRID_HMAC_SECRET to a strong random \
