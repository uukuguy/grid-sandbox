...
//! 2. Global: `~/.grid/hooks.yaml`
//! 3. Project: `$PROJECT/.grid/hooks.yaml`
//! 4. Environment: `GRID_HOOKS_FILE` override
...
pub fn resolve_hooks_path(project_dir: Option<&Path>) -> Option<PathBuf> {
    ...
    if let Ok(env_path) = std::env::var("GRID_HOOKS_FILE") {
        let p = PathBuf::from(&env_path);
        if p.exists() {
            debug!(path = %p.display(), "Using hooks config from GRID_HOOKS_FILE");
            return Some(p);
        }
        warn!(
            path = %env_path,
            "GRID_HOOKS_FILE set but file not found"
        );
    }

    // 2. Project-level config
    if let Some(project) = project_dir {
        let project_hooks = project.join(".grid").join("hooks.yaml");
    ...
    if let Some(home) = dirs::home_dir() {
        let global_hooks = home.join(".grid").join("hooks.yaml");
...
"#
    ...
    fn test_resolve_project_hooks() {
        let dir = TempDir::new().unwrap();
        let octo_dir = dir.path().join(".grid");
    ...
    fn test_resolve_no_config() {
        let dir = TempDir::new().unwrap();
        // No .grid/hooks.yaml in temp dir, and GRID_HOOKS_FILE not set
        let result = resolve_hooks_path(Some(dir.path()));
