...
mod tests {
    ...
    fn test_hook_context_to_env_vars() {
        ...
        assert!(vars.iter().any(|(k, v)| k == "GRID_SESSION_ID" && v == "s1"));
        assert!(vars.iter().any(|(k, v)| k == "GRID_SANDBOX_MODE" && v == "host"));
        assert!(vars.iter().any(|(k, v)| k == "GRID_WORKING_DIR" && v == "/tmp"));
        assert!(vars.iter().any(|(k, v)| k == "GRID_MODEL" && v == "model-x"));
        assert!(vars.iter().any(|(k, v)| k == "GRID_AUTONOMY_LEVEL" && v == "full"));
        assert!(vars.iter().any(|(k, v)| k == "GRID_TOTAL_TOOL_CALLS" && v == "10"));
        assert!(vars.iter().any(|(k, v)| k == "GRID_CURRENT_ROUND" && v == "3"));
    }
