...
mod tests {
    ...
    fn test_expand_env_vars_simple() {
        std::env::set_var("GRID_TEST_VAR_XYZ", "hello");
        assert_eq!(expand_env_vars("${GRID_TEST_VAR_XYZ}"), "hello");
        assert_eq!(expand_env_vars("pre-${GRID_TEST_VAR_XYZ}-post"), "pre-hello-post");
        std::env::remove_var("GRID_TEST_VAR_XYZ");
    }

    #[test]
    fn test_expand_env_vars_default() {
        std::env::remove_var("GRID_TEST_MISSING_VAR");
        assert_eq!(expand_env_vars("${GRID_TEST_MISSING_VAR:-fallback}"), "fallback");
        assert_eq!(expand_env_vars("${GRID_TEST_MISSING_VAR}"), "");
    }
    ...
    fn test_load_config_env_expansion() {
        std::env::set_var("GRID_TEST_API_KEY", "secret123");
        let tmp = tempfile::NamedTempFile::new().unwrap();
        writeln!(
            tmp.as_file(),
            r#"{{ "mcpServers": {{ "test": {{ "command": "cmd", "args": [], "env": {{ "API_KEY": "${{GRID_TEST_API_KEY}}" }} }} }} }}"#
        )
        .unwrap();

        let configs = McpManager::load_config(tmp.path()).unwrap();
        assert_eq!(configs[0].env.get("API_KEY").unwrap(), "secret123");
        std::env::remove_var("GRID_TEST_API_KEY");
    }
