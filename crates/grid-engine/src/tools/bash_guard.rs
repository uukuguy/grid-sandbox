...
//!
//! In container environments (GridRunMode::Sandboxed), all checks are skipped.
...
use crate::sandbox::{GridRunMode, SandboxProfile};
...
impl BashGuardLevel {
    pub fn from_context(run_mode: GridRunMode, profile: SandboxProfile) -> Self {
        match run_mode {
            GridRunMode::Sandboxed => Self::None,
            GridRunMode::Host => match profile {
...
mod tests {
    ...
    fn test_sandboxed_always_none() {
        let level =
            BashGuardLevel::from_context(GridRunMode::Sandboxed, SandboxProfile::Production);
    ...
    fn test_host_dev_is_light() {
        let level =
            BashGuardLevel::from_context(GridRunMode::Host, SandboxProfile::Development);
    ...
    fn test_host_prod_is_strict() {
        let level =
            BashGuardLevel::from_context(GridRunMode::Host, SandboxProfile::Production);
