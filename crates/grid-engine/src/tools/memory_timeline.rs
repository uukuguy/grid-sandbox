...
use grid_types::{MemoryFilter, MemoryType, SortField, ToolContext, ToolOutput, ToolSource};
...
impl Tool for MemoryTimelineTool {
    ...
    async fn execute(&self, params: Value, ctx: &ToolContext) -> Result<ToolOutput> {
        ...
        if let Some(query) = params["query"].as_str() {
            let opts = grid_types::SearchOptions {
...
fn format_search_results(entries: &[grid_types::MemoryEntry]) -> String {
...
mod tests {
    ...
    fn test_format_search_results_with_entries() {
        let entry = grid_types::MemoryEntry::new("user1", grid_types::MemoryCategory::Profile, "Test memory");
    ...
    fn test_format_episodic_entry() {
        let event = grid_types::EventData::new("create", "auth module", "success");
        let entry = grid_types::MemoryEntry::new_episodic("user1", &event, "session-123");
        let output = format_search_results(&[entry]);
