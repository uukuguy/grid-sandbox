...
fn append_log(working_dir: &std::path::Path, title: &str, message: &str) -> Result<()> {
    ...
    let log_dir = working_dir.join(".grid");
...
mod tests {
    ...
    async fn test_notify_log_channel() {
        ...
        let log_path = dir.path().join(".grid/notifications.log");
    ...
    fn test_append_log_multiple() {
        ...
        let content = std::fs::read_to_string(dir.path().join(".grid/notifications.log")).unwrap();
