...
use grid_types::SkillDefinition;
...
impl SkillLoader {
    ...
    pub fn new(project_dir: Option<&Path>, user_dir: Option<&Path>) -> Self {
        ...
        if let Some(dir) = project_dir {
            let skills_dir = dir.join(".grid").join("skills");
    ...
    fn parse_skill_from_content(path: &Path, content: String) -> Result<SkillDefinition> {
        ...
        if skill.execution_mode == grid_types::skill::ExecutionMode::Knowledge {
            let has_extra_files = std::fs::read_dir(&base_dir)
                ...
                .unwrap_or(false);
            if has_extra_files {
                skill.execution_mode = grid_types::skill::ExecutionMode::Playbook;
            }
        }
...
mod tests {
    ...
    fn create_skill_with_large_body(temp_dir: &TempDir, body_size: usize) {
        let skills_dir = temp_dir.path().join(".grid").join("skills");
...
"#,
    ...
    fn test_build_index_empty_dir() {
        let temp_dir = TempDir::new().unwrap();
        // Create empty .grid/skills directory
        let skills_dir = temp_dir.path().join(".grid").join("skills");
        std::fs::create_dir_all(&skills_dir).unwrap();
