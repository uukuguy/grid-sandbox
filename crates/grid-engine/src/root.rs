...
//! - Project root: `$PWD/.grid/` (declarative configs, project skills, can be git-tracked)
//!
//! # Directory Layout
//!
//! ```text
//! ~/.grid/                              GRID_GLOBAL_ROOT
...
//!         ├── grid.db                   SQLite (sessions, memory, tools, audit)
//!         └── history/                  Session history/snapshots
//!
//! $PWD/.grid/                           GRID_PROJECT_ROOT (declarative, git-trackable)
...
impl GridRoot {
    /// Auto-discover GridRoot from current environment.
    ///
    /// Uses `dirs::home_dir()` for global root and `std::env::current_dir()` for project root.
    /// Environment variables `GRID_GLOBAL_ROOT` and `GRID_PROJECT_ROOT` override defaults.
    pub fn discover() -> Result<Self> {
        let working_dir = std::env::current_dir().context("Failed to get current directory")?;
        Self::with_working_dir(&working_dir)
    }

    /// Create GridRoot with an explicit project directory.
    ///
    ...
    /// Create GridRoot with an explicit working directory.
    pub fn with_working_dir(working_dir: &Path) -> Result<Self> {
        let global_root = if let Ok(env_root) = std::env::var("GRID_GLOBAL_ROOT") {
        ...
        let project_root = if let Ok(env_root) = std::env::var("GRID_PROJECT_ROOT") {
            PathBuf::from(env_root)
        } else {
            working_dir.join(".grid")
        };
    ...
    pub fn db_path(&self) -> PathBuf {
        self.project_data_dir().join("grid.db")
    }

    /// Global skills directory (`~/.grid/skills/`).
    pub fn global_skills_dir(&self) -> PathBuf {
        self.global_root.join("skills")
    }

    /// Project-local skills directory (`$PWD/.grid/skills/`).
    ...
    /// Global custom commands directory (`~/.grid/commands/`).
    pub fn global_commands_dir(&self) -> PathBuf {
        self.global_root.join("commands")
    }

    /// Project-local custom commands directory (`$PWD/.grid/commands/`).
    ...
    ///
    /// Always uses `~/.grid/projects/<key>/grid.db`.
    /// `GRID_DB_PATH` env var can override for testing.
    pub fn resolve_db_path(&self) -> PathBuf {
        if let Ok(env_path) = std::env::var("GRID_DB_PATH") {
...
mod tests {
    ...
    fn test_discover_with_env_override() {
        ...
        std::env::set_var("GRID_GLOBAL_ROOT", &global);
        std::env::set_var("GRID_PROJECT_ROOT", &project);
        ...
        // Restore
        match orig_global {
            Some(v) => std::env::set_var("GRID_GLOBAL_ROOT", v),
            None => std::env::remove_var("GRID_GLOBAL_ROOT"),
        }
        match orig_project {
            Some(v) => std::env::set_var("GRID_PROJECT_ROOT", v),
            None => std::env::remove_var("GRID_PROJECT_ROOT"),
        }
    }
    ...
    fn test_ensure_dirs_and_meta() {
        ...
        let root = GridRoot::with_working_dir(&working).unwrap();
        ...
        // Restore
        std::env::remove_var("GRID_GLOBAL_ROOT");
        std::env::remove_var("GRID_PROJECT_ROOT");
    }
    ...
    fn test_with_project_dir_existing() {
        ...
        let root = GridRoot::with_project_dir(&project).unwrap();
    ...
    fn test_with_project_dir_nonexistent() {
        let result = GridRoot::with_project_dir("/nonexistent/path/that/does/not/exist");
